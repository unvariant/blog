 ...
 *
 * tcache-poisoning is used to trick malloc into returning a malloc aligned
 * arbitrary pointer by abusing the tcache freelist. (requires heap leak on and
 * after 2.32)
 *
 ...
 *
 *  *to make the PoC more reliable we need to malloc and probe the current top
 * chunk size, this should be predictable in an actual exploit and therefore,
 * can be removed to get 5 malloc calls instead
 *
 * Special Thanks to pepsipu for creating the challenge "High Frequency
 * Troubles" from Pico CTF 2024 that inspired this exploitation technique
 */
int main() {
    malloc(0x40-8);
    malloc(0x40-8);
    asm volatile("int3");
  size_t size_2, *top_size_ptr, top_size, new_top_size, freed_top_size,
      vuln_tcache, target, *heap_ptr;
  ...
  *top_size_ptr = new_top_size;
  ...
  /*
   * malloc (larger than available_top_size), to free previous top_chunk using
   * _int_free. This happens inside sysmalloc, where the top_chunk gets freed if
   * it can't be merged
   * https://elixir.bootlin.com/glibc/glibc-2.39/source/malloc/malloc.c#L2913
   * we prevent the top_chunk from being merged by lowering its size
   * we can also circumvent corruption checks by keeping PAGE_MASK bits
   * unchanged
   */
